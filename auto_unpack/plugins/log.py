...
class LogFileData(FileData):
    ...
    文件数据
    """
    stat: Optional[LogFileStat] = Field(
        default=None,
        description="文件信息"
    )
...
class LogContext(Context):
    ...
    日志类
    """
    file_datas: List[LogFileData] = Field(
        default=[],
        description="文件数据"
    )
...
class LogPlugin(Plugin[LogPluginConfig]):
    ...
    def _file_data_to_log_file_data(self, file_data: FileData) -> LogFileData:
        ...
        if 'all' in self.config.print_stats:
            for stat in stat_map.keys():
                file_info = file_info.model_copy(update={
                    stat: stat_map[stat](file_data)
                })
        else:
        ...
        return LogFileData(
            path=file_data.path,
            search_path=file_data.search_path,
            stat=file_info
        )
    ...
    def execute(self):
        ...
        write_file(log_path, log_context.model_dump_json(
